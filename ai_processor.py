...
2.  **텔레그램 HTML 형식:** 결과는 반드시 텔레그램에서 지원하는 HTML 형식에 맞춰 작성해주세요. 사용 가능한 주요 태그는 `<b>` (굵게), `<i>` (기울임꼴), `<u>` (밑줄), `<s>` (취소선), `<tg-spoiler>` (스포일러), `<code>` (고정폭 글꼴), `<pre>` (미리 서식 지정된 고정폭 코드 블록), `<a>` (하이퍼링크) 입니다. **`<ul>`, `<ol>`, `<li>`와 같은 리스트 태그나 지원 목록에 없는 다른 HTML 태그는 절대 사용하지 마십시오.** 목록을 표현해야 한다면, 각 항목 앞에 `-`, `*`, 또는 `•` 같은 글머리 기호 문자를 사용하고 줄바꿈 (`\n`)으로 구분해주세요.
...
4.  **줄바꿈:** 문단 구분을 위해서는 단순 줄바꿈 (`\n`)을 사용해주세요. 텔레그램이 이를 적절히 처리합니다. `<p>`나 `<br>` 태그는 사용하지 않아도 됩니다.
