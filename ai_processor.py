...
def summarize_for_readability(annotated_text):
    ...
    Returns:
        균형 잡힌 시각을 제공하는 최종 요약본
    """
    try:
        model = genai.GenerativeModel(GEMINI_MODEL_NAME)
        
        prompt = f"""
        다음은 비판적 분석과 주석이 추가된 뉴스 정보입니다. 이 내용을 바탕으로, 독자가 사건의 핵심을 파악하고 다양한 관점을 고려하며 균형 잡힌 시각을 가질 수 있도록 명확하고 간결하게 요약해주십시오.
        ...
        **요약 원칙:**
        ...
        3.  **객관적 어조와 중립성 유지:** 요약은 감정을 배제하고 객관적인 어조를 유지하며, 특정 입장에 치우치지 않도록 주의합니다.
        4.  **가독성 및 간결성:** 독자가 모바일 메신저 등을 통해 빠르고 쉽게 이해할 수 있도록, 간결한 문장과 적절한 문단 구분(또는 글머리 기호)을 사용합니다. 3~5개의 핵심 포인트로 정리하는 것을 목표로 합니다.
        5.  **판단 유보 및 사고 촉진:** 최종 요약은 독자에게 정답을 제시하기보다는, 다양한 정보를 바탕으로 스스로 생각하고 판단할 여지를 남겨두는 방식으로 마무리합니다. (예: "이 사안에 대해서는 다양한 해석과 평가가 존재하며, 추가적인 정보 확인이 필요해 보입니다.")
